...
import { encrypt, decrypt, encryptFields, decryptFields, PII_FIELDS } from "./encryption";
...
export class DatabaseStorage implements IStorage {
  ...
  async createUser(userData: InsertUser) {
    ...
    // Encrypt PII fields
    const encryptedData = encryptFields(userData, PII_FIELDS.user);
    ...
    // Decrypt for return
    return decryptFields(user, PII_FIELDS.user);
  }
  ...
  async updateUser(id: number, updateData: Partial<InsertUser>): Promise<User | undefined> {
    ...
    const encryptedUpdateData = encryptFields(updateData, PII_FIELDS.user);
    ...
    // Decrypt for return
    return updated ? decryptFields(updated, PII_FIELDS.user) : undefined;
  }
  ...
  async getAssets(): Promise<Asset[]> {
    ...
    const { batchDecryptFields } = await import("./encryption");
    return batchDecryptFields(assets, PII_FIELDS.asset);
  }

  async getAsset(id: number): Promise<Asset | undefined> {
    if (!db) throw new Error("Database connection required");
    const [asset] = await db.select().from(schema.assets).where(eq(schema.assets.id, id));
    return asset ? decryptFields(asset, PII_FIELDS.asset) : null;
  }
  ...
  async createAsset(assetData: InsertAsset): Promise<Asset> {
    if (!db) throw new Error("Database connection required");
    // Encrypt PII fields
    const encryptedData = encryptFields(assetData, PII_FIELDS.asset);
    const [asset] = await db.insert(schema.assets).values(encryptedData).returning();
    // Decrypt for return
    return decryptFields(asset, PII_FIELDS.asset);
  }

  async updateAsset(id: number, updateData: Partial<InsertAsset>): Promise<Asset | undefined> {
    // Encrypt PII fields before updating
    const encryptedUpdateData = encryptFields(updateData, PII_FIELDS.asset);

    const [updated] = await db.update(assets)
      .set(encryptedUpdateData)
      .where(eq(assets.id, id))
      .returning();
    return updated ? decryptFields(updated, PII_FIELDS.asset) : undefined;
  }
  ...
  async getBitlockerKeys(): Promise<any[]> {
    ...
    // Batch decrypt PII for all keys
    const { batchDecryptFields } = await import("./encryption");
    return batchDecryptFields(keys, PII_FIELDS.bitlockerKey);
  }
  ...
  async getBitlockerKey(id: number): Promise<any> {
    ...
    return key ? decryptFields(key, PII_FIELDS.bitlockerKey) : null;
  }
  ...
  async createBitlockerKey(data: any): Promise<any> {
    ...
    // Encrypt all sensitive BitLocker fields
    const encryptedData = encryptFields(data, PII_FIELDS.bitlockerKey);
    ...
    // Decrypt for return
    return decryptFields(key, PII_FIELDS.bitlockerKey);
  }

  async updateBitlockerKey(id: number, key: any): Promise<any> {
    try {
      // Encrypt PII fields before updating
      const encryptedData = encryptFields(key, PII_FIELDS.bitlockerKey);
      ...
      // Decrypt for return
      return updated ? decryptFields(updated, PII_FIELDS.bitlockerKey) : undefined;
    } catch (error) {
  ...
  async getVmInventory(): Promise<any[]> {
    try {
      ...
      // Batch decrypt PII for all VMs - uses optimized batch processing
      return batchDecryptFields(vms, PII_FIELDS.vmInventory);
    } catch (error) {
  ...
  async getVmInventoryItem(id: number): Promise<any> {
    try {
      ...
      return vm ? decryptFields(vm, PII_FIELDS.vmInventory) : undefined;
    } catch (error) {
  ...
  async createVmInventoryItem(vm: any): Promise<any> {
    try {
      ...
      const encryptedData = encryptFields(vm, PII_FIELDS.vmInventory);
      ...
      // Decrypt for return
      const decryptedVM = decryptFields(newVM, PII_FIELDS.vmInventory);
  ...
  async updateVmInventoryItem(id: number, vm: any): Promise<any> {
    try {
      ...
      // Encrypt PII fields before updating
      const encryptedUpdateData = encryptFields(updateData, PII_FIELDS.vmInventory);
      ...
      const decryptedVM = updatedVM ? decryptFields(updatedVM, PII_FIELDS.vmInventory) : undefined;
  ...
  async getITEquipment(): Promise<ITEquipment[]> {
    try {
      ...
      // Batch decrypt PII for all equipment
      const { batchDecryptFields } = await import("./encryption");
      return batchDecryptFields(equipment, PII_FIELDS.itEquipment);
    } catch (error) {
  ...
  async getITEquipmentById(id: number): Promise<ITEquipment | null> {
    ...
    return equipment ? decryptFields(equipment, PII_FIELDS.itEquipment) : null;
  }

  async createITEquipment(data: InsertITEquipment): Promise<ITEquipment> {
    try {
      // Encrypt PII fields
      const encryptedData = encryptFields(data, PII_FIELDS.itEquipment);
  ...
  async updateITEquipment(id: number, data: Partial<InsertITEquipment>): Promise<ITEquipment | null> {
    try {
      ...
      if (equipment) {
        // Decrypt for return and activity
        const decryptedEquipment = decryptFields(equipment, PII_FIELDS.itEquipment);
  ...
  async getIamAccounts(): Promise<IamAccount[]> {
    try {
      ...
      // Decrypt and map database fields (only if encryption is enabled)
  ...
  async getIamAccount(id: number): Promise<IamAccount | undefined> {
    ...
    // Decrypt and map database fields to match the expected interface
    const decryptedAccount = decryptFields(account, PII_FIELDS.iamAccount);
  ...
  async createIamAccount(data: Omit<IamAccount, 'id' | 'createdAt' | 'updatedAt'>): Promise<IamAccount> {
    ...
    // Encrypt PII fields before inserting (only if encryption is enabled)
    ...
    // Decrypt before returning to client (only if encryption is enabled)
    const decryptedAccount = process.env.ENCRYPTION_KEY
      ? decryptFields(account, PII_FIELDS.iamAccount)
      : account;
  ...
  async updateIamAccount(id: number, data: Partial<IamAccount>): Promise<IamAccount | undefined> {
    ...
    // Encrypt PII fields before updating (only if encryption is enabled)
    const encryptedData = process.env.ENCRYPTION_KEY
    ...
    // Decrypt before returning (only if encryption is enabled)
    const decryptedAccount = process.env.ENCRYPTION_KEY
