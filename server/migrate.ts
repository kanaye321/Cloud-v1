...
export async function runMigrations() {
  try {
    ...
    async function addMissingVMInventoryColumns() {
      ...
      // Specific check for critical fields
      if (!(await columnExists('vm_inventory', 'requestor'))) {
        await addColumn('vm_inventory', 'requestor', 'TEXT');
        console.log("✅ Added missing requestor column");
      }
      if (!(await columnExists('vm_inventory', 'jira_number'))) {
        await addColumn('vm_inventory', 'jira_number', 'TEXT');
        console.log("✅ Added missing jira_number column");
      }
    ...
    // Add mfaEnabled, mfaSecret, and forcePasswordChange columns if they don't exist
    try {
      await db.execute(sql`
        DO $$ 
        BEGIN
        ...
        END $$;
      `);
      console.log('✅ MFA and password management columns added/verified in users table');
    } catch (mfaError) {
      console.error('⚠️ Error adding MFA/password columns:', mfaError);
    }
    ...
    try {
      // Insert default email notification settings if they don't exist
      ...
      if (!existingSettings) {
        ...
        console.log('✅ Default system settings created with email notifications enabled');
      } else {
        // Update existing settings to ensure notification fields exist
        console.log('Updating existing settings to ensure all notification fields are present...');
        await db.update(systemSettings)
          .set({
            notifyOnIamExpiration: existingSettings.notifyOnIamExpiration ?? true,
            notifyOnVmExpiration: existingSettings.notifyOnVmExpiration ?? true,
          })
          .where(eq(systemSettings.id, existingSettings.id));
        console.log('✅ System settings updated with notification defaults');
      }
    } catch (settingsError) {
      console.error("⚠️ Error checking/creating system settings:", settingsError.message);
    }
