...
export interface IStorage {
  ...
  getIamAccount(id: number): Promise<any | undefined>;
  updateIamAccount(id: number, data: any): Promise<any | undefined>;
  deleteIamAccount(id: number): Promise<boolean>;
  importIamAccounts(accounts: any[]): Promise<any>;
}
    ...
    for (const account of accounts) {
      try {
        if (this.isMemoryStorage) {
          ...
          await this.db.run(
            `INSERT INTO iam_accounts (requestor, knox_id, permission, duration_start_date, duration_end_date, cloud_platform, project_accounts, approval_id, remarks, status) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [account.requestor, account.knoxId, account.permission, account.durationStartDate, account.durationEndDate, 
             account.cloudPlatform, account.projectAccounts, account.approvalId, account.remarks, account.status || 'active']
          );
        }
        results.successful++;
      } catch (error: any) {
