...
import {
  ...
  insertSystemSettingsSchema, insertVmInventorySchema, insertVmApprovalHistorySchema, insertBitlockerKeySchema, insertIamAccountSchema, insertMonitorSchema, insertItEquipmentSchema, insertVMMonitoringSchema, insertDiscoveredHostSchema, insertMonitoringDashboardSchema, insertMonitoringDatasourceSchema, insertMonitoringAlertRuleSchema, insertMonitoringPanelSchema,
  systemSettings, AssetStatus,
  LicenseStatus, AccessoryStatus, users, itEquipment, monitorInventory, vmInventory, iamAccounts, bitlockerKeys, vms, vmApprovalHistory, monitoringDashboards, monitoringDatasources, monitoringAlertRules, monitoringPanels, consumableAssignments, consumables, networkDiscoveryHosts, itEquipmentAssignments
} from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/check-iam-expirations", requireAuth, async (req: Request, res: Response) => {
    try {
      console.log('📧 Manual check for expired IAM accounts triggered by:', req.user?.username);

      // Log the manual check action
      await logEmailEvent({
        timestamp: new Date().toISOString(),
        to: 'system',
        subject: `Manual IAM expiration check triggered by ${req.user?.username}`,
        status: 'success'
      });

      const settings = await storage.getSystemSettings();

      console.log('IAM expiration check - settings:', {
        notifyOnIamExpiration: settings?.notifyOnIamExpiration,
      ...
      if (settings?.notifyOnIamExpiration === false) {
        await logEmailEvent({
          timestamp: new Date().toISOString(),
          to: 'system',
          subject: 'Manual IAM expiration check',
          status: 'failed',
          error: 'IAM expiration notifications are disabled in settings'
        });
      ...
      console.log(`IAM Expiration Check: Found ${expiredAccounts.length} expired accounts out of ${iamAccounts.length} total`);
      ...
      if (expiredAccounts.length > 0) {
        ...
        console.log(`Sending notifications for accounts:`, accountsData.map(a => a.knoxId));

        // Log before sending notifications
        await logEmailEvent({
          timestamp: new Date().toISOString(),
          to: 'system',
          subject: `Attempting to send notifications for ${expiredAccounts.length} expired IAM account(s)`,
          status: 'success'
        });
        ...
        for (const account of expiredAccounts) {
          ...
          console.log(`Updated account ${account.id} status to expired_notified`);
        }
        ...
        if (emailSent) {
          ...
          await logEmailEvent({
            timestamp: new Date().toISOString(),
            to: 'system',
            subject: `Successfully sent notifications for ${expiredAccounts.length} expired IAM account(s)`,
            status: 'success'
          });
        } else {
          // Log failure
          await logEmailEvent({
            timestamp: new Date().toISOString(),
            to: 'system',
            subject: `Failed to send notifications for ${expiredAccounts.length} expired IAM account(s)`,
            status: 'failed',
      ...
      await logEmailEvent({
        timestamp: new Date().toISOString(),
        to: 'system',
        subject: 'Manual IAM expiration check - No expired accounts found',
        status: 'success'
      });
      ...
      await logEmailEvent({
        timestamp: new Date().toISOString(),
        to: 'system',
        subject: 'Manual IAM expiration check failed',
        status: 'failed',
        error: err.message
      });
  ...
  app.get("/api/email-logs", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const logs = lines.reverse().map(line => {
          const timestampMatch = line.match(/\[(.*?)\]/);
          const statusMatch = line.match(/\[(SUCCESS|FAILED|PENDING)\]/);
      ...
      return res.status(500).json({
        message: `Failed to read email logs: ${err.message}`
      });
    }
  });
      ...
      for (const host of hostsData.result) {
        try {
          ...
          const vmData = {
            ...
            memoryUsage: memoryItem ? parseFloat(item.lastvalue) : null, // Corrected to use memoryItem.lastvalue
  ...
  app.put("/api/iam-accounts/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Updating IAM account with ID: ${id} and data:`, updateData);
      ...
      // Use storage layer which handles timestamps and encryption properly
      const updatedAccount = await storage.updateIamAccount(id, updateData);
  ...
  app.post("/api/vm-inventory", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const mappedVMData = {
        ...
        diskGB: vmData.diskGB || vmData.diskCapacityGB || null,
  ...
  app.post("/api/consumables/:id/assign", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const newQuantity = consumable.quantity - quantityToAssign;
    ...
    try {
      ...
      try {
        ...
        const tablesToBackup = tables && tables.length > 0 ? tables : [
          ...
          'iam_accounts', 'monitor_inventory', 'vm_approval_history'
        ];
